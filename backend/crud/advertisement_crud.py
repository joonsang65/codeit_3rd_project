...
def create_advertisement(db: Session, user_id: int, description: str) -> DBAdvertisement:
    """
    광고를 데이터베이스에 생성합니다.
    ...
    """
    new_ad = DBAdvertisement(user_id=user_id, description=description)
    db.add(new_ad)
    db.commit()
    db.refresh(new_ad)
    return new_ad

def update_advertisement(db: Session, ad_entry: DBAdvertisement, new_description: Optional[str] = None) -> DBAdvertisement:
    """
    광고 정보를 업데이트합니다.
