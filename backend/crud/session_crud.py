...
# ****************************************** 세션 생성 ******************************************
def create_session(db: Session, session_id: str, session_data: dict, user_id: Optional[str] = None, user: Optional[DBUser] = None) -> DBSession:
    """
    세션을 데이터베이스에 생성합니다.
...
def update_session_data(db: Session, session_entry: DBSession, new_data: dict) -> DBSession:
    ...
    """
    import json
    session_entry.session_data = json.dumps(new_data) if isinstance(new_data, dict) else new_data
    db.add(session_entry)
    db.commit()
    db.refresh(session_entry)
    return session_entry
