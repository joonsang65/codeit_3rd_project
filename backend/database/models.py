...
class Session(SQLModel, table = True):
    ...
    session_data: Dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSON), description="Session data stored as a dictionary.") # JSON 형식으로 저장될 수 있음.

    # 상위 클래스와의 관계를 정의.
    user: Optional["User"] = Relationship(back_populates="sessions", sa_relationship_kwargs={"lazy": "selectin"})
    user_id: Optional[int] = Field(default=None, foreign_key="users.id", description="Foreign key to the user.")
...
class Advertisement(SQLModel, table = True):
    ...
    user_id: int = Field(default=None, foreign_key="users.id", description="Foreign key to the user who created the advertisement.")
