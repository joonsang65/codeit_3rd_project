...
def delete_advertisement(ad_id: int, db: Annotated[Session, Depends(get_session)]) -> None:
    """특정 광고를 삭제합니다."""
    try:
...
def delete_ad_image_generation(image_gen_id: int, db: Annotated[Session, Depends(get_session)]) -> None:
...
def delete_ad_image_preservation(image_pres_id: int, db: Annotated[Session, Depends(get_session)]) -> None:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"이미지 보존 요청 삭제 실패: {e}")
...
def delete_ad_copy(copy_id: int, db: Annotated[Session, Depends(get_session)]) -> None:
    """특정 광고 문구 기록을 삭제합니다."""
    try:
